...
public class InternalsVerifier
    ...
    public void caseAInternal(
        ...
        for(MacroVersion version : this.globalIndex.getAllVersions()) {
            MacroInfo macroInfo = this.globalIndex.getMacro(this.currentMacroInfo.getNameDeclaration(), version);

            if(macroInfo != this.currentMacroInfo) {
                if(!macroInfo.containsParam(expected_param)){
                    throw CompilerException.missingParameter(macroInfo.getNameDeclaration(), version, expected_param);
                }
                ...
                if(expected_param.isString()
                    ...
                    throw CompilerException.incorrectParameterType(found_param, macroInfo.getNameDeclaration(), version, Collections
                    ...
                    if(!found_types.containsAll(expected_types)){
                        throw CompilerException.incorrectParameterType(found_param, macroInfo.getNameDeclaration(), version, expected_types);
                    }
                }
            }
        }
