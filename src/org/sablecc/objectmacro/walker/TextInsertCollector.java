...
import org.sablecc.objectmacro.structure.Scope;
...
public class TextInsertCollector
    ...
    public void outATextInsert(
        ...
        TextInsert textInsert = this.currentScope.getTextInsert(node);
        if (node.getStaticValues().size() != textInsert.getInsertedTextBlock()
                .getDeclaration().getParams().size()) {
            throw CompilerException.incorrectArgumentCount(textInsert);
        }
    }
